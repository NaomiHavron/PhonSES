...
sound_files = ["ko_chien_48000.wav", "ka_poussette_48000.wav", "ko_lapin_48000.wav", "ka_tracteur_48000.wav", "ko_cochon_48000.wav", "ka_biberon_48000.wav", "ko_souris_48000.wav", "ka_chaussure_48000.wav", "ko_poisson_48000.wav", "ka_chapeau_48000.wav", "ko_cheval_48000.wav", "ka_velo_48000.wav"]
images = ["Chien_Image.jpg", "Poussette_Image.jpg", "Lapin_Image.jpg", "Tracteur_Image.jpg", "Cochon_Image.jpg", "Biberon_Image.jpg", "Souris_Image.jpg", "Chaussure_Image.jpg"]

#Group together images, fixed
N=2
pairs=[images[n:n+N] for n in range(0,len(images), N)]
pairs=[tuple(i) for i in pairs]
dataset={i:list() for i in pairs}


# Build images and audios pairs
i = 0
for k, v in dataset.items():
    dataset[k] = [sound_files[i],sound_files[i+1]]
    i+=2
...
def Train():
    ...
    for i in range(0,2):
        ...
        while i<2:
            ...
            # like at the second loop targets change
            # for key in randomizedlist:
            #
            #tgtsound=dataset[k][tgt]
            for k,v in dataset.items():
                # get target sound
                tgt_sound=v[i]
                # file formatting
                tgt_pair=str(k).strip("()").replace("'","").split(", ")
                print(tgt_pair,tgt_sound)
                Left= tgt_pair[0]
                Image_Left.image = tgt_pair[0]
                Image_Left.opacity = 1
                Image_Left.draw()
                Right=tgt_pair[1]
                Image_Right.image = tgt_pair[1]
                Image_Right.opacity = 1
                Image_Right.draw()
                Window.flip()
                timer = core.Clock()
                Momes=timer.getTime()
                tagEvent( "TRIALSTART",particNum, trial, Momes,Left, Right, "tgt")
                trialInit(trial)
                ...
                timer = core.Clock()
                Momes=timer.getTime()
                tagEvent("TRIALEND",particNum, trial, Momes,Left, Right, "tgt")
...
# core.wait(1)
# subprocess.Popen(["python", "C:/Users/barbir/Desktop/TheBridge/videoPlayer.py"])
# core.wait(3)
# Window.close()
# Window = visual.Window(fullscr=True,monitor=mon,size=[1280, 720],units="deg") #the images are adjusted
